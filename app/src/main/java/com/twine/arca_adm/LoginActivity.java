...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    @Background
    void request_Auth(String username, String password){
        String respuesta = restClient.get_empleado(username,password);
        try {
            JSONObject jrespuesta = new JSONObject(respuesta);
            if(jrespuesta.getInt("code")==200){
                new Update(Empleado.class).set("activo=?",false).execute();
                JSONObject jempleado = jrespuesta.getJSONObject("empleado");

                Empleado empleado=new Select().from(Empleado.class)
                        .where("id_empleado=?",jempleado.getInt("id"))
                        .executeSingle();
                if (empleado==null)
                    empleado = new Empleado();
                empleado.id_empleado=jempleado.getInt("id");
                empleado.nombre=jempleado.getString("nombre");
    ...
    @UiThread
    void respuestaLogin(Boolean success){
        mAuthTask = null;
        showProgress(false);
